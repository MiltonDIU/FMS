{{--
    ...
    A full copy on purpose. Ledger owns every file it renders, so deleting
    another theme cannot take this one down with it — there is no partial
    outside theme_ledger/ that any page here needs.

    The seeded platform list is 25 entries, and the researcher import writes
    ORCID, Scopus, Web of Science and Website among them, so a directory that
    knew only the handful of consumer networks would show one recognisable icon
    and four identical chain links on a scholar's profile.
    ...
    an invented one is worse than the letters.
...
{{-- The brand colour is handed over as a custom property rather than set as
     ...
     These are the colours the platforms publish, and every one of them is
     chosen against white. Set as an inline `color` they could not be corrected
     from a stylesheet without !important; as a custom property they can, which
     is what lets the theme lift them wherever it puts them on a dark ground. --}}
@if(isset($paths[$key]))
