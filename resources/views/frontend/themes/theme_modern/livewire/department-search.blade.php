                ...
                <ul class="border-l border-[#A7A9AC]" role="list">
                    <li>
                        <button type="button" wire:click="toggleAll"
                            class="block w-full text-left border-l-[3px] px-3 py-2 rounded-none text-[15px] font-medium transition-colors {{ $this->all ? 'bg-[#EDF6FF] border-diu-primary text-slate-900' : 'border-transparent text-slate-700 hover:bg-[#EDF6FF]' }}">
                            All Faculties
                        </button>
                    </li>
                    @php
            ...
            <div class="flex flex-wrap items-start justify-between gap-4">
                ...
                @if($this->department->faculty)
                    <a href="{{ route('department.contact', ['faculty_short_name' => strtolower($this->department->faculty->short_name), 'department_code' => strtolower($this->department->code)]) }}"
                       wire:navigate
                       class="inline-flex items-center gap-2 bg-diu-primary hover:bg-diu-primary-dark text-white text-sm font-semibold px-4 py-2.5 rounded-2xl transition-colors shadow-sm shrink-0">
