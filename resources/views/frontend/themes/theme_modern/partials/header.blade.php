    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <div class="flex items-center gap-3 justify-between md:justify-end">
                <div class="hidden lg:flex items-center gap-3 bg-slate-50 border border-slate-200/60 rounded-2xl p-1.5 pl-3">
                    ...
                    <span class="bg-diu-primary/10 text-diu-primary font-mono font-bold text-xs px-3 py-1.5 rounded-xl border border-diu-primary/10 shadow-2xs">
                        {{ $teachersCount }} {{ $brand['scholars_label'] }}
                    </span>
                </div>

                @if(auth()->check())
                    <a href="{{ url('/admin') }}" class="bg-diu-primary hover:bg-diu-primary-hover text-white text-xs font-bold px-4 py-2.5 rounded-2xl transition-all duration-200 flex items-center gap-2 shadow-sm border border-diu-primary/25 active:scale-95">
                        <svg class="w-3.5 h-3.5 text-diu-accent" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="18" height="11" x="3" y="11" rx="2"/><circle cx="12" cy="7" r="4"/></svg>
                        <span>Dashboard</span>
                    </a>
                @else
                    <a href="{{ url('/admin/login') }}" class="bg-diu-primary hover:bg-diu-primary-hover text-white text-xs font-bold px-4 py-2.5 rounded-2xl transition-all duration-200 flex items-center gap-2 shadow-sm border border-diu-primary/25 active:scale-95">
