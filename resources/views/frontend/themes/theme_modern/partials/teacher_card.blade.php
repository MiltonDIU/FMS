...
@if($isAdmin)
    <div class="group flex flex-col card card-hover rounded-3xl overflow-hidden">
        <div class="flex-1">
                    ...
                    @if($areaCount > 0)
                        <div class="flex flex-wrap items-center gap-1 pt-1">
                            @foreach($teachingAreas->take(2) as $ta)
                                <span class="bg-slate-100 text-slate-600 text-[9px] font-sans px-2 py-0.5 rounded-full">{{ $ta->area }}</span>
                            @endforeach
                            @if($areaCount > 2)
                                <span class="bg-slate-100 text-slate-400 text-[8px] font-sans font-bold px-1.5 py-0.5 rounded-full">+{{ $areaCount - 2 }}</span>
                            @endif
                        </div>
                    @endif
                </div>
                ...
                @if(count($interests) > 0)
                    <div class="mt-3 flex flex-wrap gap-1">
                        @foreach(array_slice($interests, 0, 2) as $interest)
                            <span class="bg-diu-accent/10 text-diu-accent text-[9px] font-sans px-2 py-0.5 rounded-full font-medium">{{ $interest }}</span>
                        @endforeach
                        @if(count($interests) > 2)
                            <span class="bg-slate-100 text-slate-400 text-[8px] font-sans font-bold px-1.5 py-0.5 rounded-full">+{{ count($interests) - 2 }}</span>
                        @endif
                    </div>
                @endif
            </div>
        ...
        <div class="flex-1">
            ...
            <div class="pt-10 px-5">
                ...
                @if(count($interests) > 0)
                    <div class="mt-3 flex flex-wrap gap-1">
                        @foreach(array_slice($interests, 0, 2) as $interest)
                            <span class="bg-diu-primary/10 text-diu-primary text-[9px] font-sans px-2 py-0.5 rounded-full font-medium">{{ $interest }}</span>
                        @endforeach
                        @if(count($interests) > 2)
                            <span class="bg-slate-100 text-slate-400 text-[8px] font-sans font-bold px-1.5 py-0.5 rounded-full">+{{ count($interests) - 2 }}</span>
                        @endif
