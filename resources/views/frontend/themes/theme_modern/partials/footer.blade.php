@php
    ...
    $brand = Branding::all();
    $matchFooter = filter_var(\App\Models\Setting::get('theme_theme_modern_footer_match_theme', false), FILTER_VALIDATE_BOOLEAN);
@endphp
                            ...
                            @if(! empty($url))
                                <a href="{{ $url }}" target="_blank" rel="noopener noreferrer"
                                   ...
                                   title="{{ ucfirst($link['platform'] ?? '') }}">
                                    @include("frontend.themes.theme_modern.partials.social_icon", ['platform' => $link['platform'] ?? 'website'])
                                </a>
                            @endif
                        @endforeach
                    </div>
