...
@endphp
    ...
    <div class="card overflow-hidden font-sans">
        ...
        <div class="p-6 md:p-8 space-y-8">
            ...
            <!-- Core Metadata Block -->
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 bg-slate-50 rounded-2xl border border-slate-100 text-xs ring-1 ring-slate-900/5">
                @if($venue)
                    <div>
            ...
            <div x-data="{ copied: null, doCopy(ref, key) { const el = $refs[ref]; if(!el) return; navigator.clipboard.writeText(el.innerText); copied = key; setTimeout(() => copied = null, 2000); } }"
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <pre x-ref="bibtex" class="p-3 bg-slate-900 text-slate-100 rounded-xl text-[11px] font-mono select-all overflow-x-auto whitespace-pre leading-normal shadow-inner">{{ $citations['bibtex'] }}</pre>
                    </div>
                </div>
            </div>
