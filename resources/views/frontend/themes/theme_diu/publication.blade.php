@php
    ...
    $year = $publication->publication_year ?? 'n.d.';
    ...
    $bibtex = "@article{diu_{$pubId},\n  author = {{$authors}},\n  title = {{$title}},\n  journal = {{$venue}},\n  year = {{$year}}\n}";
@endphp

<!DOCTYPE html>
<html lang="en">
<head>
    @include('frontend.themes.theme_diu.partials.head', ['title' => $publication->title . ' - Publication Details'])
</head>
<body class="bg-transparent min-h-screen flex flex-col font-sans text-slate-800 antialiased">
    ...
    <main class="flex-1 max-w-5xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 md:py-8">
        ...
        <div class="text-xs text-slate-500 font-semibold mb-8 flex flex-wrap items-center gap-2 glass-panel py-2.5 px-5 rounded-2xl">
            <a href="{{ url('/') }}" class="hover:text-diu-green transition">Home</a>
            ...
            <svg class="w-3.5 h-3.5 text-gray-300" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m9 18 6-6-6-6"/></svg>
            <a href="{{ url('/' . strtolower($faculty->short_name) . '/' . strtolower($department->code)) }}" class="hover:text-diu-green transition">{{ $department->code }}</a>
            <svg class="w-3.5 h-3.5 text-gray-300" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m9 18 6-6-6-6"/></svg>
            <a href="{{ url('/' . strtolower($faculty->short_name) . '/' . strtolower($department->code) . '/' . $teacher->webpage) }}" class="hover:text-diu-green transition">{{ $teacher->first_name }} {{ $teacher->last_name }}</a>
            <svg class="w-3.5 h-3.5 text-gray-300" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m9 18 6-6-6-6"/></svg>
            <span class="text-diu-green truncate max-w-xs">Publication Details</span>
        </div>

        <div class="bg-white/40 backdrop-blur-md rounded-2xl border border-white/60 shadow-xs overflow-hidden font-sans">

            <!-- Upper header -->
            <div class="bg-gradient-to-r from-diu-green to-diu-green-dark p-6 md:p-8 text-white relative">
                <a href="{{ url('/' . strtolower($faculty->short_name) . '/' . strtolower($department->code) . '/' . $teacher->webpage) }}"
                ...
                <span class="bg-diu-orange text-white text-[10px] font-sans font-bold uppercase px-2.5 py-0.5 rounded-sm tracking-wide">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 bg-white/30 backdrop-blur-xs rounded-xl border border-white/60 text-xs ring-1 ring-slate-900/5">
                    ...
                    <div>
                        ...
                        <p class="font-semibold text-slate-800 mt-1 flex items-center gap-1">
                            <svg class="w-3.5 h-3.5 text-diu-green" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="18" height="18" x="3" y="4" rx="2"/><path d="M8 2v4M16 2v4M3 10h18"/></svg>
                ...
                @if($publication->abstract)
                    <div>
                        <h3 class="text-xs font-bold text-slate-800 uppercase tracking-wider mb-2.5 flex items-center gap-1.5">
                            <svg class="w-4 h-4 text-diu-green" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><path d="M14 2v6h6M16 13H8M16 17H8M10 9H8"/></svg>
                            Abstract
                        </h3>
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            <div class="flex justify-between items-center mb-1 text-[11px] font-semibold text-gray-400">
                                <span>APA STYLE</span>
                                <button @click="doCopy('apa', 'apa')" class="hover:text-diu-green flex items-center gap-1 cursor-pointer transition-colors">
                            ...
                            <p x-ref="apa" class="p-3 bg-white/50 border border-white/80 rounded-lg text-xs text-slate-700 font-sans select-all leading-relaxed">{{ $apa }}</p>
                        </div>
                        ...
                        <div>
                            <div class="flex justify-between items-center mb-1 text-[11px] font-semibold text-gray-400">
                                ...
                                <button @click="doCopy('ieee', 'ieee')" class="hover:text-diu-green flex items-center gap-1 cursor-pointer transition-colors">
                            ...
                            <p x-ref="ieee" class="p-3 bg-white/50 border border-white/80 rounded-lg text-xs text-slate-700 font-sans select-all leading-relaxed">{{ $ieee }}</p>
                        </div>
                        ...
                        <div>
                            <div class="flex justify-between items-center mb-1 text-[11px] font-semibold text-gray-400">
                                ...
                                <button @click="doCopy('bibtex', 'bibtex')" class="hover:text-diu-green flex items-center gap-1 cursor-pointer transition-colors">
                            ...
                            <pre x-ref="bibtex" class="p-3 bg-slate-900 text-slate-100 rounded-lg text-[11px] font-mono select-all overflow-x-auto whitespace-pre leading-normal shadow-inner">{{ $bibtex }}</pre>
                        </div>
                    </div>
                </div>
                ...
                <div class="p-4 bg-white/30 border border-white/60 rounded-xl flex items-center justify-between ring-1 ring-slate-900/5">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-full overflow-hidden bg-slate-200 shrink-0">
                            ...
                            @else
                                <div class="w-full h-full bg-diu-green text-white flex items-center justify-center font-display font-bold">{{ strtoupper(substr($teacher->first_name, 0, 1)) }}</div>
                            @endif
                        </div>
                        <div>
                    ...
                    <a href="{{ url('/' . strtolower($faculty->short_name) . '/' . strtolower($department->code) . '/' . $teacher->webpage) }}"
                       class="text-xs font-semibold text-diu-green hover:text-diu-orange transition-colors flex items-center gap-1">
