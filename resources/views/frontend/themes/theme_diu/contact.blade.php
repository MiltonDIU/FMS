...
@section('content')
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <!-- Cover / Hero header banner (matches profile & publication pages) -->
        <div class="relative h-48 bg-gradient-to-r from-diu-primary-dark via-diu-primary to-diu-accent/80 p-6 md:p-8 flex items-end border-b border-slate-200/60">
            @if(! empty($sections['department']['department_photo']))
                <img src="{{ $sections['department']['department_photo'] }}" alt="{{ $sections['department']['department_name'] ?? $department->name }}" class="absolute inset-0 w-full h-full object-cover opacity-20 mix-blend-overlay" />
            @endif
            <a href="{{ $deptUrl }}"
