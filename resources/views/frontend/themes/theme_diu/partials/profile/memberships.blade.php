            ...
            @foreach($membershipList as $mem)
                <div class="p-3  rounded-xl border border-slate-200 flex items-center gap-2.5 text-xs text-slate-700 font-sans font-medium ring-1 ring-slate-900/5">
                    ...
                    @php
                        ...
                        // is_active decides whether it is still held. A missing
                        // end date means nobody recorded one, not that the
                        ...
                        // position and the membership type often hold the same
                    ...
                    <div class="min-w-0">
                        <span class="font-semibold text-slate-800">{{ optional($mem->membershipOrganization)->name ?: ($kind ?: 'Membership') }}</span>
                        @if($facts)
                            <span class="text-slate-500">&middot; {{ implode(' · ', $facts) }}</span>
                        @endif
                        @if($period)
