...
<div x-show="tab === 'awards'" class="space-y-4" x-cloak>
    ...
    @else
        {{-- By year, newest first. year is the column that is filled; date is
             there for the records that carry a full one instead, and both are
             resolved once so the sort and the heading cannot disagree. --}}
        @php
            $awardYear = fn ($awr) => $awr->year
        ...
        <div class="space-y-6">
          @foreach($years as $year => $rows)
            ...
            @foreach($rows as $awr)
                ...
                @endphp
                <div class="p-4 rounded-xl border border-diu-accent/20 bg-diu-accent/5 backdrop-blur-xs flex gap-3.5 items-start">
                    <div class="bg-white text-diu-accent p-2 rounded-lg shrink-0 border border-diu-accent/10 shadow-3xs">
