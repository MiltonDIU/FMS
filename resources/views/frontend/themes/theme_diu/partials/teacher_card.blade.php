@php
    $isAdmin = optional($teacher->designation)->name
        && preg_match('/(dean|head|chairman|director|coordinator|advisor)/i', optional($teacher->designation)->name);
    $accent = $isAdmin ? 'diu-orange' : 'diu-green';
    $initials = strtoupper(substr($teacher->first_name, 0, 1) . substr($teacher->last_name, 0, 1));
    $interests = $teacher->research_interest
        ? array_filter(array_map('trim', explode(',', $teacher->research_interest)))
        : [];
    $profileUrl = url('/' . strtolower($faculty->short_name) . '/' . strtolower($department->code) . '/' . $teacher->webpage);
@endphp

<div class="bg-white/40 backdrop-blur-md rounded-xl border transition-all duration-300 overflow-hidden group flex flex-col justify-between cursor-pointer ring-1 {{ $isAdmin ? 'border-diu-orange/30 hover:border-diu-orange/60 ring-diu-orange/10 hover:ring-diu-orange/25 shadow-sm' : 'border-white/60 hover:border-white/95 ring-diu-green/10 hover:ring-diu-green/25 shadow-sm' }}">
    <div>
        <!-- Card Header Top Accent color -->
        <div class="h-1.5 {{ $isAdmin ? 'bg-diu-orange' : 'bg-diu-green' }}"></div>
        ...
        <div class="p-5">
            ...
            <div class="flex items-start gap-4">
                ...
                <div class="relative w-16 h-16 shrink-0 rounded-full overflow-hidden border-2 border-white shadow-md">
                    ...
                    @else
                        <div class="w-full h-full bg-diu-green text-white flex items-center justify-center font-display font-bold text-lg">
                            {{ $initials }}
                        </div>
                    @endif
                </div>
                ...
                <div class="flex-1 min-w-0">
                    <div class="flex flex-wrap items-center gap-1.5 mb-1">
                        @if($isAdmin)
                            <span class="inline-flex items-center gap-1 bg-diu-orange/15 text-diu-orange text-[9px] font-sans font-bold uppercase px-1.5 py-0.5 rounded-sm border border-diu-orange/25">
                    ...
                    <h4 class="text-sm font-semibold text-slate-800 tracking-tight leading-snug line-clamp-1 group-hover:text-diu-green transition-colors">
            ...
            <div class="mt-5 space-y-2 border-t border-white/40 pt-3">
                <div class="flex items-center gap-2 text-xs text-slate-600">
                    ...
                    <span class="truncate hover:text-diu-green transition-colors font-mono">{{ $teacher->secondary_email ?? 'N/A' }}</span>
                </div>
                <div class="flex items-center gap-2 text-xs text-slate-600">
    ...
    <a href="{{ $profileUrl }}" class="px-5 py-3.5 bg-white/30 border-t border-white/40 flex items-center justify-between group-hover:bg-white/60 transition-colors">
        <div class="flex items-center gap-1.5">
            <svg class="w-4 h-4 {{ $isAdmin ? 'text-diu-orange' : 'text-diu-green' }}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 10v6M2 10l10-5 10 5-10 5z"/><path d="M6 12v5c3 3 9 3 12 0v-5"/></svg>
            <span class="text-[10px] text-slate-400 font-semibold uppercase font-sans">
                {{ $teacher->publications->count() }} Publications
            </span>
        </div>
        <span class="text-xs font-semibold text-diu-green group-hover:text-diu-orange flex items-center gap-1 transition-all">
            Profile
