    ...
    <div>
        ...
        <div class="p-5">
            ...
            <div class="mt-5 space-y-2 border-t border-white/40 pt-3">
                ...
                @if($areaCount > 0)
                    <div class="flex items-center gap-2 text-xs text-slate-600">
                          <span class="truncate font-sans leading-tight">
                            @foreach($teachingAreas->take(2) as $ta)
                                <span class="bg-white/40 text-slate-600 border border-white/60 text-[9px] font-sans px-2 py-0.5 rounded-sm">
                                      {{ $ta->area }}@if(! $loop->last)@endif
                                </span>


                            @endforeach
                            @if($areaCount > 2)
                                <span class="bg-white/50 text-slate-400 text-[8px] font-sans font-bold px-1.5 py-0.5 rounded-sm border border-white/60">+{{ $areaCount - 2 }}</span>
                            @endif
                        </span>
                    </div>
                @endif
    ...
    <a href="{{ $profileUrl }}" wire:navigate class="px-5 py-3.5 bg-white/30 border-t border-white/40 flex items-center justify-between group-hover:bg-white/60 transition-colors">
        <div class="flex items-center gap-1.5">
            ...
            <span class="text-[10px] text-slate-400 font-semibold uppercase font-sans">
                {{ $teacher->publications_count ?? $teacher->publications->count() }} Publications
            </span>
        </div>
