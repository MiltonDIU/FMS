            ...
            ->toolbarActions([
                BulkActionGroup::make([
                    ...
                    BulkAction::make('mergeSelected')
                        ...
                        // rows, so it needs Delete:Position, not just list access.
                        ...
                        ->form(fn (\Illuminate\Support\Collection $records) => [
                            \Filament\Forms\Components\Select::make('target_id')
                                ->label('Select Primary / Target Position')
                        ...
                        ->action(function (\Illuminate\Support\Collection $records, array $data) {
                            ...
                            \Illuminate\Support\Facades\DB::transaction(function () use ($targetId, $targetRecord, $sourceIds) {
                                ...
                                \App\Models\Position::whereIn('id', $sourceIds)->delete();
                            });

                            // Update cached file dynamically
                            app(\App\Services\DuplicateFinderService::class)->removeGroupFromCache('position', $targetId, $sourceIds);
