...
export default function PublicationDetailPage({ params }) {
  ...
  if (loading) {
    return (
      <div class="container mx-auto py-20 text-center text-sm font-semibold text-gray-400">
  ...
  if (!publication || !teacher) {
    return (
      <div class="container mx-auto py-20 text-center">
        <h2 class="text-xl font-bold">Publication Not Found</h2>
        <Link href={`/${faculty}/${deptCode}/${teacherWebpage}`} class="text-blue-600 hover:underline mt-4 inline-block">Back to Profile</Link>
      </div>
    );
  }
    ...
    <div class="min-h-screen bg-slate-50 py-12">
      <div class="container mx-auto max-w-5xl px-4">
        ...
        <div class="text-xs text-gray-500 font-semibold mb-8 flex flex-wrap items-center gap-2">
            ...
            <span>/</span>
            <Link href={`/${faculty}`} class="hover:text-blue-600 transition uppercase">{faculty}</Link>
            <span>/</span>
            <Link href={`/${faculty}/${deptCode}`} class="hover:text-blue-600 transition uppercase">{deptCode}</Link>
            <span>/</span>
            <Link href={`/${faculty}/${deptCode}/${teacherWebpage}`} class="hover:text-blue-600 transition">{teacher.name}</Link>
            <span>/</span>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="bg-white rounded-3xl border border-gray-100 p-6 shadow-sm space-y-6">
              ...
              <div class="space-y-4">
                {publication.journal_name && (
                  <div>
                    <p class="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Journal</p>
              ...
              {publication.paper_link && (
                <div class="border-t border-gray-100 pt-6">
                  <a href={publication.paper_link} target="_blank" class="w-full py-3 bg-blue-900 hover:bg-blue-800 text-white rounded-2xl text-center font-bold text-xs uppercase tracking-wider block transition duration-300">
          ...
          <div class="lg:col-span-2">
            <div class="bg-white rounded-3xl border border-gray-100 p-8 shadow-sm space-y-6">
              <span class="px-2.5 py-1 bg-blue-50 text-blue-800 text-[10px] font-bold rounded-lg uppercase tracking-wide">
                Research Paper
              </span>
                ...
                <div class="border-t border-gray-100 pt-6">
                  ...
                  <div class="flex flex-wrap gap-2">
                    {publication.keywords.split(',').map((kw, i) => (
                      <span key={i} class="px-3 py-1 bg-slate-50 border border-gray-100 rounded-lg text-xs font-medium text-gray-600">
                        {kw.trim()}
                      </span>
                    ))}
                  </div>
